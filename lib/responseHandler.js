...
module.exports = function(config) {
    return function*(next) {
        ...
        if (this.method === 'GET' && isAssignHost && isOpenDebug) {
			...
			//添加gfe全局变量
            pageContent = pageContent.replace('<\/head>', '<script>var gfeJsCdn = "//'+config.staticServerHostname+':'+config.staticServerPort+'",gfeJsCdn = "",gfeProjectPath = "";</script>\r\n<\/head>');
...
function generateNewUrl(config, singleUrl, urlType) {
    var localServerHost = '//' + config.staticServerHostname + ':' + config.staticServerPort + '/';
    ...
    singleUrl = singleUrl.slice(singleUrl.indexOf(mountFolder + '/'));
